...
function loadUsers() {
    axios.get(`${apiBase}/list`).then(response => {
        users.value = (response.data || []).filter(u => u && u.id);
    }).catch(() => {
...
function saveUser() {
    ...
    if (user.username?.trim() && user.email?.trim()) {
        const saveAction = user.id
            ? axios.put(`${apiBase}/edit/${user.id}`, user)
            : axios.post(`${apiBase}/add`, user);
...
function editUser(u) {
    axios.get(`${apiBase}/view/${u.id}`).then(response => {
...
function deleteUser() {
    axios.delete(`${apiBase}/delete/${user.id}`).then(() => {
...
function deleteSelectedUsers() {
    const deletePromises = selectedUsers.value.map(u => axios.delete(`${apiBase}/delete/${u.id}`));
...
<template>
    <div class="card">
        ...
        <DataTable ref="dt" v-model:selection="selectedUsers" :value="users" dataKey="id" :paginator="true" :rows="10"
            ...
            <Column header="Sr No." :body="(_, { index }) => index + 1" style="width: 8rem" />
            <Column field="username" header="User Name" sortable />
            <Column field="email" header="Email" sortable />
            <Column field="cpf_no" header="CPF" sortable />
            <Column field="user_type" header="Type" sortable />
        ...
        <!-- Create/Edit Dialog -->
       <Dialog v-model:visible="userDialog" :draggable="false" modal header="User Details" :closable="false" style="width: 60vw">
  <div class="p-4">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div>
        <label class="font-bold mb-1 block">Username</label>
        <InputText v-model="user.username" class="w-full" />
      </div>
  ...
  <template #footer>
    ...
    <Button label="Create User" icon="pi pi-check" class="p-button-primary" @click="saveUser" />
  </template>
</Dialog>
