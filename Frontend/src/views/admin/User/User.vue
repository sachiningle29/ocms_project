...
function saveUser() {
  ...
  const request = user.id
    ? axiosClient.put(`${apiBase}/${user.id}`, payload)
  ...
  <div class="card">
    <Toolbar class="mb-4">
      <template #start>
        ...
        <Button label="Delete" icon="pi pi-trash" severity="secondary" @click="confirmDeleteSelected" :disabled="!selectedUsers.length" />
      </template>
    </Toolbar>
