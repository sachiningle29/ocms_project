...
const filters = ref({
  status: 'ongoing',
  chartType: 'donut',
  applyTo: [...props.initialFilters.applyTo]// Array to store which charts to apply filters to
});
  ...
  <div class="relative">
    ...
    >
      ...
      <div v-if="!isCollapsed" class="space-y-4">
        ...
        <!-- Chart Selection -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Apply to Charts:</label>
          <div class="space-y-2">
            <div v-for="chart in availableCharts" :key="chart.id" class="flex items-center">
              <input
                type="checkbox"
                :id="`chart-${chart.id}`"
                :value="chart.id"
                v-model="filters.applyTo"
                @change="applyFilters"
                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
              >
              <label :for="`chart-${chart.id}`" class="ml-2 block text-sm text-gray-700">
                {{ chart.name }}
              </label>
            </div>
          </div>
