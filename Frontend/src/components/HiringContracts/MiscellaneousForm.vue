<template>
    <fieldset class="border rounded p-4">
        <legend class="font-semibold text-lg mb-2">Execution</legend>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            ...
            <!-- Vendor Name -->
            <div class="flex flex-col">
                <label for="contractor_name" class="font-bold mb-1 block">Vendor Name</label>
                <InputText v-model="contract.contractor_name" class="w-full" />
            </div>

            <!-- Vendor Code -->
            <div class="flex flex-col">
                <label for="vendor_code" class="font-bold mb-1 block">Vendor Code*</label>
                <InputText id="vendor_code" v-model="contract.vendor_code" class="w-full"
                    :class="{ 'p-invalid': fieldErrors.vendor_code }" maxlength="6" @input="validateVendorCode" />
                <small v-if="fieldErrors.vendor_code" class="p-error">Vendor Code must be exactly 6 digits</small>
            </div>
        ...
        <div class="mt-6">
            ...
            <div class="grid grid-cols-12 gap-2 items-center mb-3">
                ...
                <div class="col-span-2">
                    <!-- <InputText v-model="contract.contract_start_date_expected_date" type="date" class="w-full" /> -->
                </div>
                <div class="col-span-2">
                    <InputText v-model="contract.contract_start_date_actual_date" type="date" class="w-full" />
                </div>
            ...
            <div class="grid grid-cols-12 gap-2 items-center">
                ...
                <div class="col-span-2">
                    <InputText v-model="contract.contract_end_date_expected_date" type="date" class="w-full" />
                </div>
                <div class="col-span-2">
                    <InputText v-model="contract.contract_end_date_actual_date" type="date" class="w-full" />
                </div>
                <div class="col-span-4">
...
export default {
    ...
    methods: {
        validateVendorCode() {
            const code = this.contract.vendor_code;
            // Ensure it's exactly 6 digits
            const isValid = /^\d{6}$/.test(code);
            this.fieldErrors.vendor_code = !isValid;
        }
    }
}
</script>